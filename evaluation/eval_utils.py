...
def get_next_word_predictions(
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        if batch_size == 1:
            ...
            choice_indices = (
                torch.sum(batch_input_ids != tokenizer.pad_token_id, dim=1) - 1
            )
            batch_logits = model(
                input_ids=batch_input_ids, attention_mask=attention_mask
            ).logits
            choice_indices = choice_indices.view(batch_logits.shape[0], 1, 1).expand(
                batch_logits.shape[0], 1, batch_logits.shape[2]
            )
