...
def main(args):
    ...
    for task_name in tqdm.tqdm(all_tasks.keys(), desc="Evaluating"):
        ...
        if args.model_name_or_path:
            ...
            # generate with hf model
                ...
                outputs = generate_completions(
                    ...
                    # we only use stop token for non-chat format (usually applied to vanilla pretrained language models). For chat format, we will rely on the model knows when to stop.
                    stop_id_sequences=(
                        [[stop_sequence]] if not args.use_chat_format else None
                    ),
                )
        else:
            instances = []
            ...
            results = query_openai_chat_model(
                ...
                output_path=os.path.join(
                    args.save_dir,
                    "predictions",
                    f"{task_name}_openai_prediction_cache.jsonl",
                ),
            )
        ...
        with open(
            os.path.join(args.save_dir, "predictions", f"{task_name}.jsonl"), "w"
        ) as fout:
            for example in task_examples:
    ...
    with open(os.path.join(args.save_dir, "metrics.json"), "w") as fout:
        performance["average_exact_match"] = sum(performance.values()) / len(
            performance
        )
