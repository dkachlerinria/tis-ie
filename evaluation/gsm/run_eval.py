...
from evaluation.gsm.examplars import EXAMPLARS as GSM_EXAMPLARS
...
def main(args):
    ...
    if args.model_name_or_path:
        ...
        if args.use_vllm:
            ...
            if args.use_chat_format:
                prompts = [
                    apply_chat_format(example, tokenizer) for example in test_data
                ]
            else:
                prompts = [
                    prompt_prefix
        ...
        instances = [
            {"id": prompt, "prompt": prompt} for _, prompt in enumerate(prompts)
        ]
