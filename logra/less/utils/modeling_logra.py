...
class LoGra:
    ...
    Example:
        >>> model = LoGra.from_pretrained("Qwen/Qwen3-0.6B-Base")
    ...
    def _format_item(self, item: Dict[str, Any]) -> str:
        ...
        - {'input': ..., 'output': ..., 'pre_formatted': True}  ← bypass all reformatting
        """
        # Pre-formatted inputs (from our pipeline) skip the AirRep reformatting step.
        if item.get('pre_formatted'):
    ...
    def encode(
        ...
        for i in data_iter:
            ...
            # Clear gradients and cache to prevent OOM
    ...
    def _precondition(self, embeds: torch.Tensor, batch_size: int = 512, damping: float = None) -> torch.Tensor:
        ...
        # pinv handles near-singular FIMs gracefully (important for small sample counts)
        fim_inv = torch.linalg.pinv(fim_damped)
